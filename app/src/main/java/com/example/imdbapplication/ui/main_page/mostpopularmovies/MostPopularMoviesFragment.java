...
public class MostPopularMoviesFragment extends Fragment {
    ...
    @Inject
    public MostPopularsMovieViewModel viewModel;
    ...
    private void setData() {
        viewModel.getMostPopularMovies().observe(getActivity(), new Observer<ItemsList<Movie>>() {
            @Override
            public void onChanged(ItemsList<Movie> mostPopularMovies) {
                adapter.setMovieList(mostPopularMovies.getItems());
            }
        });
        title.setText(R.string.title_most_popular_movies);
    }
